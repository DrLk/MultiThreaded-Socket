...
FileCache::BufferView FileTree::GetData(std::uint64_t inode, off_t offset, size_t size) const
{
    const auto& leaf = inode == RootInode ? *_root : *(reinterpret_cast<const Leaf*>(inode)); // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast, performance-no-int-to-ptr)
    return leaf.GetData(offset, size);
}

FileTree::Data FileTree::AddData(std::uint64_t inode, off_t offset, size_t size, Data&& data)
{
    const int packetsNumber = static_cast<int>(data.size());
    auto& leaf = inode == RootInode ? GetRoot() : *(reinterpret_cast<Leaf*>(inode)); // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast, performance-no-int-to-ptr)
...
FreeData FileTree::GetFreeData(size_t shardIdx)
{
    ...
    const std::uint64_t inode = entry->first;
    auto& leaf = inode == RootInode ? GetRoot() : *(reinterpret_cast<Leaf*>(inode)); // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast, performance-no-int-to-ptr)
