...
public:
    ...
    {
        return sendto(_socket, buffer.data(), buffer.size(), 0, reinterpret_cast<const struct sockaddr*>(&addr), sizeof(sockaddr_in)); // NOLINT
    }

    size_t RecvFrom(std::span<unsigned char> buffer, sockaddr_storage& addr) const
    {
        socklen_t len = sizeof(sockaddr_storage);
        int receivedBytes = recvfrom(_socket, buffer.data(), buffer.size(), 0, reinterpret_cast<struct sockaddr*>(&addr), &len); // NOLINT
